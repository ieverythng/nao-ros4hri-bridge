...
- Before the first serious graph pass on this repo, run `scripts/bootstrap_socialminds_sources.sh` so GitNexus can see the ignored ROS4HRI source packages and reference KB stack.
...
- For live UI sessions: if the GitNexus HTTP backend is already running, prefer `GITNEXUS_USE_HTTP=1 tools/knowledge/codex_with_gitnexus.sh`. In that mode, `codex mcp list` will show the shared backend as `gitnexus_http`, and you should avoid direct local CLI graph calls against the same running repo database.
