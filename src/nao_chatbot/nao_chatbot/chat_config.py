#!/usr/bin/env python3
"""Parameter declaration and configuration loading for chat skill server."""
...
from nao_chatbot.skill_catalog import parse_package_list


INTENT_DETECTION_MODES = {"rules", "llm", "llm_with_rules_fallback"}

DEFAULT_PROMPT_ADDENDUM = (
...
class ChatSkillConfig:
    """Runtime configuration for ChatSkillServer and turn engine."""
...
def declare_chat_parameters(node) -> None:
    """Declare all node parameters required by chat skill server."""
    node.declare_parameter("action_name", "/skill/chat")
    node.declare_parameter("role_name", "__default__")
    node.declare_parameter("enabled", True)
    node.declare_parameter("ollama_url", "http://localhost:11434/api/chat")
    ...
    node.declare_parameter("robot_name", "Pop")
    node.declare_parameter("persona_prompt_path", "")
    node.declare_parameter("system_prompt", "")
    node.declare_parameter("prompt_addendum", DEFAULT_PROMPT_ADDENDUM)
...
def load_chat_config(node) -> ChatSkillConfig:
    ...
    raw_prompt_addendum = str(node.get_parameter("prompt_addendum").value).strip()
    ...
    return ChatSkillConfig(
        action_name=str(node.get_parameter("action_name").value),
        role_name=str(node.get_parameter("role_name").value),
        enabled=as_bool(node.get_parameter("enabled").value),
        ollama_url=str(node.get_parameter("ollama_url").value),
        model=model,
        intent_model=intent_model,
        request_timeout_sec=float(node.get_parameter("request_timeout_sec").value),
        first_request_timeout_sec=float(
            node.get_parameter("first_request_timeout_sec").value
        ),
        intent_request_timeout_sec=max(
            0.5,
            float(node.get_parameter("intent_request_timeout_sec").value),
        ),
        context_window_tokens=int(node.get_parameter("context_window_tokens").value),
        ...
        max_history_messages=max(0, int(node.get_parameter("max_history_messages").value)),
        ...
        identity_reminder_every_n_turns=int(
            node.get_parameter("identity_reminder_every_n_turns").value
        ),
...
def as_bool(value) -> bool:
    """Coerce bool-ish values from ROS params."""
