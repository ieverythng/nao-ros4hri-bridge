#!/usr/bin/env python3
"""Conversation-history utilities for chat skill server."""
...
def history_to_messages(history_entries, max_history_messages: int) -> list[dict]:
    """Convert serialized history entries ("role:text") to Ollama messages."""
    messages = []
...
def messages_to_history(messages: list[dict], max_history_messages: int) -> list[str]:
    """Serialize Ollama messages to compact role-prefixed history entries."""
    serialized = []
