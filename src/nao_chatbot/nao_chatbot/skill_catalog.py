...
class SkillDescriptor:
    """Compact skill descriptor to inject in prompts."""
...
def parse_package_list(value: str) -> list[str]:
    """Parse CSV package list into normalized names."""
    tokens = []
...
def build_skill_catalog_text(
    ...
    """Build compact catalog text from allowlisted packages."""
...
def _parse_skill_manifest_yaml(payload: str, logger=None) -> dict[str, Any]:
    ...
    # Fallback extractor for environments without PyYAML.
    manifest = {}
    id_match = re.search(r"^\s*id:\s*(.+?)\s*$", payload, flags=re.MULTILINE)
    if id_match:
        manifest["id"] = id_match.group(1).strip()

    path_match = re.search(
        r"^\s*default_interface_path:\s*(.+?)\s*$",
        payload,
        flags=re.MULTILINE,
    )
    if path_match:
    ...
    description_match = re.search(
        r"^\s*description:\s*\|\s*$([\s\S]*?)^\s*[a-zA-Z_]+:\s*",
        payload,
        flags=re.MULTILINE,
    )
    ...
    names = re.findall(r"^\s*-\s*name:\s*(.+?)\s*$", payload, flags=re.MULTILINE)
    if names:
