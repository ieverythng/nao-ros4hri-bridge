...
def _assert_lab_vllm_defaults(defaults: dict[str, str]) -> None:
    ...
    assert defaults["chatbot_temperature"] == "0.2"
    assert defaults["chatbot_top_p"] == "0.9"
    assert defaults["chatbot_top_k"] == "0"
    assert defaults["chatbot_min_p"] == "0.0"
    assert defaults["chatbot_presence_penalty"] == "0.0"
    assert defaults["chatbot_repetition_penalty"] == "1.0"
    assert defaults["planner_llm_temperature"] == "0.1"
    assert defaults["planner_llm_top_p"] == "1.0"
    assert defaults["planner_llm_top_k"] == "0"
    assert defaults["planner_llm_min_p"] == "0.0"
    assert defaults["planner_llm_presence_penalty"] == "0.0"
    assert defaults["planner_llm_repetition_penalty"] == "1.0"
    assert defaults["chatbot_intent_request_timeout_sec"] == "10.0"
