...
def build_report_outcome(
    *,
    ...
    summary = plan_outcome_summary if isinstance(plan_outcome_summary, dict) else {}
    for target in _clean_list(summary.get('completed_targets', [])):
        if any(item.get('id') == target for item in reportable_objects):
            continue
        if _target_can_be_report_object(target, entity_index, location_index):
            _append_unique(
                reportable_objects,
                _entity_contract(target, entity_index=entity_index, status='completed'),
            )
        else:
            _append_unique(
                excluded_targets,
                _excluded_contract(
                    ...
                    entity_index=entity_index,
                    location_index=location_index,
                    reason=_excluded_reason(target, entity_index, location_index),
                ),
            )
...
def _report_mode(
    *,
    ...
    if reportable_objects and recipients:
        return 'delivery'
