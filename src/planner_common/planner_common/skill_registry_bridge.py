...
def merge_fake_skill_aliases(
    *,
    ...
    for skill_payload in manifest:
        if not _is_fake_skill_payload(skill_payload):
            continue
        canonical = _fake_skill_canonical_name(skill_payload)
        if not canonical:
            continue
...
def _is_fake_skill_payload(skill_payload: dict) -> bool:
    if bool(skill_payload.get('is_fake', False)):
        return True
    mapping = str(skill_payload.get('robot_adapter_mapping', '')).strip().lower()
    if not mapping:
        return False
    mapping_prefix = mapping.split('.', 1)[0]
    return mapping_prefix.startswith('fake_') or mapping.startswith('fake_skills.')
...
def _fake_skill_canonical_name(skill_payload: dict) -> str:
    ...
    return str(skill_payload.get('name', '')).strip().lower()
