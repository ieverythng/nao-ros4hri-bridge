...
def discover_repo_root() -> pathlib.Path:
    """Resolve repo root from cwd, script location, or git top-level."""
    search_roots: list[pathlib.Path] = []
    cwd = pathlib.Path.cwd().resolve()
    here = pathlib.Path(__file__).resolve().parent
    search_roots.extend([cwd, *cwd.parents, here, *here.parents])
    ...
    try:
        ...
        git_root = ""
    if git_root:
        git_path = pathlib.Path(git_root).resolve()
        search_roots.extend([git_path, *git_path.parents])
    ...
    for candidate in search_roots:
        if candidate in seen:
            continue
        seen.add(candidate)
        if (candidate / "src" / "nao_chatbot" / "package.xml").exists():
            return candidate
    raise RuntimeError(
        "Could not locate repository root (expected src/nao_chatbot/package.xml). "
        f"Checked from cwd={cwd} and script={pathlib.Path(__file__).resolve()}."
    )
...
def print_report(packages: dict[str, PackageInfo]) -> int:
    ...
    skill_refs = REPO_ROOT / ".codex" / "skills" / "iiia-ros4hri-check" / "references"
    print("Next reads:")
    print(f"  - {skill_refs / 'repo-boundaries.md'}")
    print(f"  - {skill_refs / 'ros4hri-guardrails.md'}")
    return 0
