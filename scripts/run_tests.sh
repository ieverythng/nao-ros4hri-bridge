...
for pattern in (
    ...
    "src/nao_world_model_enricher/nao_world_model_enricher/*.py",
...
echo "[4/10] planner and WME package unit tests"
PYTHONPATH="src/planner_common:src/planner_llm:src/nao_world_model_enricher:${PYTHONPATH:-}" python3 -m pytest -q \
  src/planner_common/test/test_contracts.py \
  src/planner_llm/test/test_planner_engine.py \
  src/nao_world_model_enricher/test/test_world_state.py
...
if have_python_module numpy; then
  ...
  PYTHONPATH="src/planner_common:src/nao_world_model_enricher:src/kb_skills:src/nao_orchestrator:${PYTHONPATH:-}" python3 -m pytest -q \
    src/nao_orchestrator/test/test_nao_orchestrator_intent_rules.py
fi
