...
\begin{enumerate}
  ...
  \item expose and discretize robot capabilities through a reviewed skill registry instead of direct robot-specific APIs;
  ...
  \item implement execution feedback and feedback-driven planning so the system can handle failed steps, ambiguous targets, clarification requests, replanning, and changes in the world state;
...
\begin{enumerate}
  ...
  \item \textbf{RQ3:} Does a reviewed skill registry, combined with deterministic validation, reduce invalid planner outputs such as unsupported skills, malformed arguments, or robot-specific API calls?
