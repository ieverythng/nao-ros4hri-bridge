...
\section{Selected Evidence Sets}

Each evidence set fixes its launch mode, model or source configuration, case set, artifact, and validation status. Scores from different tuples are not averaged because a model can produce fluent dialogue while reducing planner schema validity or recovery reliability. Case metrics are the primary results; the runtime-review score is retained as a secondary run summary.
...
F08 & \code{response_first}, JSON-only grounding & Fake/replan & Four eight-case deterministic policy sweeps through tracked-voice and \code{LiveSpeech} ingress, with isolated confirmation probes. & Historical, 8.6\\
...
\section{Full Integrated Runtime Results}

F28 exercised the response-first speech path from tracked voice and \code{LiveSpeech} through dialogue, planning, deterministic execution, feedback, and final speech. Table~\ref{tab:full-runtime-results} reports the main runtime boundaries checked in that run.
...
Table~\ref{tab:interaction-contract-results} summarises the interaction and architecture checks applied across the response-first evidence.
...
Table~\ref{tab:rq-result-synthesis} gives a qualified answer to each research question. Each row includes both the positive results and the failure cases that limit generalisation.
...
\section{Stateful KB Stress Diagnostic}

Evidence set F15 evaluated the Ollama \code{qwen3-coder:480b-cloud} backend with response-first routing. The run inserted two named objects, revised a support relation, exercised grounded delivery, and queried postconditions. TITAS and MIDAS were available with their names, colours, and supports. The revised TITAS support was used on the next turn. Both delivery cases selected the expected object and ALEX, and direct post-execution queries observed the declared recipient relation.

The diagnostic also exposed two pre-limit semantic counterexamples. One quantified delivery reached planner handoff with an empty selected-member set. A separate request to bring a kitchen cup to the user selected the kitchen as recipient and reported delivery to the kitchen. This user-visible false completion applies the 6/10 critical-failure cap. The first explicit provider HTTP 429 occurred at 03:01:31 CEST. Later model-dependent cases and the unstarted multi-step deterministic fake-skill matrix are marked not scored rather than treated as semantic regressions. The focused artifact is retained as qualified evidence because the provider cutoff prevented a complete single-tuple run.
