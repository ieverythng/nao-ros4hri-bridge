...
3A & Shared skill entry
& Registry projection consumed by the planner, orchestrator, and skill servers.
& \code{skill_common} defines the shared capability shape; \code{planner_llm} uses it for prompt construction and \code{nao_orchestrator} uses it for admission and dispatch.\\
...
\section{Contract 3A: Shared Skill Entry}
...
\begin{lstlisting}
{
  ...
  "category": "navigation",
  "aliases": ["go_to"],
  "params": {"target": "entity or location identifier"},
  "required_params": ["target"],
  "preconditions": ["target is grounded or scenario-owned"],
  "expected_effects": ["robot arrives at the target location"],
  "observable_success": ["status=success", "arrival evidence"],
  "failure_modes": ["path_blocked", "unknown_location"],
  "planner_guidance": ["use for a single destination"],
  "result_schema": {"required": ["skill", "status", "summary_text"]},
  "safety_flags": ["navigation"],
  "runtime_available": true,
  "implementation_status": "fake",
  "executor": "fake_skills.navigate_to"
}
\end{lstlisting}
...
\midrule
\code{name}, \code{category}, \code{aliases}
...
\code{safety_flags}, \code{runtime_available}
& Safety-relevant labels and profile-specific availability checks.\\
\code{implementation_status}, \code{executor}
& Whether the selected profile has an implementation and which adapter or skill server receives the call.\\
\bottomrule
\end{tabularx}
\end{table}
