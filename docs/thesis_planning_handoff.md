...
deterministic downstream executor for robot skills, and a first `planner_llm`
scaffold is now live behind launch flags on `feat/TFM-LLM_planner`. The next
stage is to evolve this from grounded single-turn interaction into a modular
planning architecture where a planner layer can reason over multi-intent input,
query and revise the KB, generate executable plans, react to failures, and
replan through structured feedback from the orchestrator without tying the
design to NAO-specific logic.
