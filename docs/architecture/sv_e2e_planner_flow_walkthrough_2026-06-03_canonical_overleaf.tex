...
\begin{itemize}
\item The planner must not prefill \code{report\_result.args.summary\_text} after \code{scan}, \code{perform\_motion}, \code{look\_at}, \code{navigate\_to}, or another executable skill.
...
\item \code{summary\_text} is only allowed for standalone reports of known facts that were not produced by a previous plan step.
\end{itemize}
