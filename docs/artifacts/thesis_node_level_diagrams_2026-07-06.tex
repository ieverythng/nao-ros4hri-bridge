...
\begin{tikzpicture}[node distance=8mm and 11mm]
  ...
  \node[dialogueNode, right=of turn, minimum width=30mm] (history) {Conversation state\\role, locale,\\recent turns};
  ...
  \node[groundNode, below=of prompt, minimum width=32mm] (context) {\code{grounded_context_v3}\\entities, locations,\\bounded relations};
...
\begin{tikzpicture}[node distance=8mm and 11mm]
  ...
  \node[groundNode, below=of request, minimum width=34mm] (ctx) {Planner context\\\code{grounded_context_v3}\\scene targets};
  ...
  \node[plannerNode, below=of prompt, minimum width=32mm] (supervisor) {Goal supervisor\\active goal\\replan / clarify};
...
\caption{\code{planner_llm} node-level view. The planner generates and supervises structured plan envelopes; execution and speech remain downstream responsibilities.}
...
\begin{tikzpicture}[node distance=9mm and 12mm]
  ...
  \node[skillNode, right=of dispatcher, minimum width=31mm] (skills) {Skill/adaptor calls\\AB=1 actions\\fake profiles};
...
\begin{tikzpicture}[node distance=8mm and 10mm]
  ...
  \node[dialogueNode, right=of kbskills, minimum width=32mm] (projection) {Chatbot projection\\\code{knowledge_snapshot}\\\code{grounded_context_v3}};
