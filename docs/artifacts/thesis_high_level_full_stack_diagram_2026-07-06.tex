...
\begin{tikzpicture}[x=1mm,y=1mm]
  ...
  \node[actor] (environment) at (10,60) {Environment\\camera, simulator,\\preloaded KB};
  ...
  \node[font=\scriptsize\bfseries, anchor=west] at (99,100.6) {Planning and supervision};
  \node[execNode] (gate) at (113,84) {\code{nao_orchestrator}\\planner gate\\admission};
  \node[plannerNode] (planner) at (144,84) {\code{planner_llm}\\plan generation\\supervision};
  ...
  \node[bus, minimum width=184mm] (plannerBus) at (127,69) {
  ...
  \node[groundingNode] (scene) at (48,40) {\code{nao_scene_grounding}\\detector facts\\\code{/scene/summary}};
  \node[groundingNode, text width=22mm] (kb) at (79,40) {\code{kb_skills}\\KnowledgeCore\\query/revise};
  \node[groundingNode] (context) at (110,40) {\code{grounded}\\\code{context_v3}\\entities, locations\\bounded relations};
  ...
  \node[font=\scriptsize\bfseries, anchor=west] at (129,56.6) {AB=1 runtime skills and validation seams};
  ...
  \node[skillNode, text width=39mm] (review) at (102,-14) {\code{robot-runtime}\\\code{performance-review}\\main + fake\_deep};
  \node[futureNode, text width=34mm] (wme) at (148,-14) {World Model\\Enricher\\future compact\\memory};
  \node[futureNode, text width=39mm] (neural) at (194,-14) {Neural Workbench\\AB>=2 decomposition\\promotion gates};
  ...
  \draw[adapter] (orchestrator.east) -- ++(15,0) |- ([xshift=-8mm]skillsArea.north east);
  ...
  \draw[future] (neural.north) -- (skillsArea.south);
