...
\begin{tikzpicture}[node distance=8mm and 11mm]
  ...
  \node[plannerNode, right=of gate, minimum width=30mm] (planner) {\code{planner_llm}\\plan generation\\supervision};
  ...
  \node[groundingNode, right=of kb, minimum width=34mm] (context) {\code{grounded_context_v3}\\entities, locations,\\bounded relations};

  \node[futureNode, below=13mm of kb, minimum width=38mm] (wme) {Future: world model\\enricher / richer VLM\\scene semantics};
  \node[futureNode, below=13mm of fake, xshift=-8mm, minimum width=40mm] (nw) {Future: Neural Workbench\\AB>=2 decomposition\\promotion gates};
  ...
  \begin{pgfonlayer}{background}
    ...
    \node[layerBox, fit=(scan)(report)(motion)(look)(fake), label={[font=\small\bfseries]below:AB=1 runtime skills and validation seams}] {};
  \end{pgfonlayer}
\end{tikzpicture}%
}
\caption{Implemented ROS4HRI planner stack. Dialogue, planning, deterministic execution, symbolic grounding, and AB=1 skills remain separate runtime owners. Dashed boxes mark future implementation directions rather than current execution authority.}
...
\begin{tikzpicture}[node distance=9mm and 13mm]
  ...
  \node[skillNode, right=of validate, minimum width=28mm] (skill) {6. Skill result\\AB=1 action};

  \node[plannerNode, below=13mm of plan, minimum width=30mm] (supervise) {7. Supervise\\replan, clarify,\\complete, fail};
  ...
  \draw[feedbackFlow] (skill) |- node[pos=0.25, right, font=\scriptsize]{typed result} (supervise);
  \draw[feedbackFlow] (validate) |- node[pos=0.27, left, font=\scriptsize]{plan/step status} (supervise);
  \draw[thesisFlow] (supervise) -- node[below, font=\scriptsize]{dialogue act} (speak);
  \draw[feedbackFlow] (supervise) to[bend right=20] node[above, font=\scriptsize]{revised plan when needed} (validate);
\end{tikzpicture}%
}
...
\begin{tikzpicture}[node distance=7mm]
  ...
  \node[groundingNode, below=of smoke, minimum width=95mm] (env) {Preloaded environment: RDF fixture, \code{/kb/query}, \code{grounded_context_v3}};
  ...
  \node[execNode, below=of fake, minimum width=95mm] (deep) {Deep fake/replan suite: grouped delivery, missing target, recipient missing, fail-once recovery};
...
This separation is important for duplicate-speech prevention. \code{planner_llm} can request acknowledgement, clarification, failure explanation, or completion notification through \code{/planner/dialogue_act}, but the user-facing utterance is realized through the dialogue/speech layer. As a result, the planner can supervise execution without becoming a speaking node, and the orchestrator can report status without inventing conversational wording.
...
The orchestrator is downstream-only. It can reject a plan, block unsafe continuation, dispatch supported actions, and publish \code{/planner/execution_feedback}; it does not generate replacement plans and does not become an LLM policy node. This is the main safety seam between language reasoning and robot actuation. Real \code{walk_to} promotion is deliberately narrow: the real route maps to \code{/skill/move_to} with bounded body-relative \code{x_m}, \code{y_m}, and \code{theta_rad}; semantic target navigation remains fake or future decomposition work.

\subsection*{Grounding and Knowledge}

The grounding path converts perception and KnowledgeCore state into compact symbolic evidence. \code{nao_scene_grounding} owns detector normalization, detector-derived object facts, optional spatial overlay merging, and \code{/scene/summary}. \code{kb_skills} owns the query/revise transport boundary to KnowledgeCore. \code{chatbot_llm} projects those sources into \code{grounded_context_v3}, whose main fields are \code{entities}, \code{locations}, bounded relations, counts, and freshness metadata.

The planner receives this compact T0 context for reasoning, but the context is not treated as proof that an execution-time effect succeeded. AB=1 skills remain responsible for fresh evidence at execution time. This distinction prevents image-plane detections, stale KB facts, or planning-time summaries from being reported as completed robot actions.
...
This layer is not a replacement for robot validation. It is a controlled seam for testing route correctness, registry exposure, plan validation, execution feedback, clarification, safe failure, and replanning. The deep fake/replan suite should therefore be reported separately from the main response-first runtime score.
...
Grounded context & Inserted or preloaded KB facts appear in \code{/kb/query}, \code{grounded_context_v3}, and the chatbot/planner turn trace.\\
