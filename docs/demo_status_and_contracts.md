...
The stack now demonstrates six major capabilities working together:
...
6. `nao_world_model_enricher` is now available on the research branch as a
   planner-facing action-conditioned context layer built from `/scene/summary`,
   optional KB rows, and executor feedback.
...
| WME planner-context layer | live on the research branch | gives the planner a short-horizon world model rather than raw scene summaries alone |
