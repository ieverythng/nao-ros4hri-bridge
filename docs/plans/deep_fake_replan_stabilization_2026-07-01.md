...
| Grouped delivery from named locations | Still mixed in the last runtime artifact. The rebuilt `lab_sections` delivery prompt clarified on "work table" even though the fixture contains `dbp:name work_table`. | Source now preserves non-label location aliases and the planner matcher reads them before clarification. Rerun the exact work-table delivery prompt after rebuild. |
...
| Skill-bound KB post-effects | Delivered objects gained the human relation but could retain stale source locations. | Fake `bring_object` now removes known-source `oro:isOn`, `oro:isAt`, and `oro:isIn` relations before adding the recipient relation. Rerun grouped delivery followed by a KB location query. |
